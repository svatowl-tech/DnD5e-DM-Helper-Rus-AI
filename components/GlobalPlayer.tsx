...
const GlobalPlayer: React.FC = () => {
    ...
    return (
        <div className="w-full h-12 bg-gray-900/95 border-t border-gold-600/30 backdrop-blur-md flex items-center px-4 justify-between select-none z-40">
            ...
            <div className="flex items-center gap-2 shrink-0">
                ...
                <button 
                    onClick={togglePlay}
                    disabled={!currentTrack} 
                    className={`p-2 rounded-md transition-all flex items-center justify-center mx-1 ${isPlaying ? 'bg-gold-600/20 text-gold-500' : 'text-gray-300 hover:bg-gray-800'}`}
                >
                    {isLoading ? <Loader className="w-4 h-4 animate-spin"/> : isPlaying ? <Pause className="w-4 h-4 fill-current"/> : <Play className="w-4 h-4 fill-current ml-0.5"/>}
                </button>
                ...
                <div className="relative" ref={volumeRef}>
                    ...
                    {/* Volume Popover */}
                    {showVolume && (
                        <div className="absolute bottom-full right-0 mb-2 p-3 bg-gray-900 border border-gray-700 rounded-lg shadow-xl w-10 flex flex-col items-center gap-2 animate-in fade-in zoom-in-95 origin-bottom-right">
